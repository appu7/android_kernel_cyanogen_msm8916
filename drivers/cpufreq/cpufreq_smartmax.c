...
static unsigned int suspend_ideal_frequ;
...
static ssize_t show_suspend_ideal_frequ(struct kobject *kobj, struct attribute *attr,
		char *buf) {
	return sprintf(buf, "%u\n", suspend_ideal_frequ);
}

static ssize_t store_suspend_ideal_frequ(struct kobject *kobj, struct attribute *attr,
		const char *buf, size_t count) {
	ssize_t res;
	unsigned long input;
	res = strict_strtoul(buf, 0, &input);
	if (res >= 0 && input >= 0) {
		suspend_ideal_frequ = input;
		if (is_suspended){
			ideal_freq = suspend_ideal_frequ;
			smartmax_update_min_max_allcpus();
		}
	} else
		return -EINVAL;
...
define_global_rw_attr(suspend_ideal_frequ);
...
static int __init cpufreq_smartmax_init(void) {
	...
	up_rate = 30000;
	down_rate = 60000;
	suspend_ideal_frequ = 422400;
	awake_ideal_freq = 1267200;
	ideal_freq = awake_ideal_freq;
	ramp_up_step = 200000;
